...
int
cc_array_chain_add_elem (struct cc_array_chain *self, void *elem)
{
  if (self->cursor == NULL)
    {
...
int
cc_array_chain_append (struct cc_array_chain *self, void *data, size_t nums)
{
  size_t i;
  for (i = 0; i < nums; i++)
    {
...
int
cc_array_chain_node_size (struct cc_array_chain *self, size_t index)
{
  size_t n;
...
int
cc_array_chain_to_array (struct cc_array_chain *self, struct cc_array **result,
                         int nums_to_reserve)
{
...
int
cc_array_chain_new (struct cc_array_chain **self, int node_elem_nums,
                    int node_elem_size)
{
...
int
cc_array_chain_delete (struct cc_array_chain *self)
{
  struct cc_list_iter iter;
  struct cc_array **tmp;
...
int
cc_array_chain_node_new (struct cc_array_chain *self)
{
  struct cc_array *arr;
