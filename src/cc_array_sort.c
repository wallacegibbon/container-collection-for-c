...
///-----------------------------------------------------------------------------
static size_t
cc_array_divide (struct cc_array *self, cc_cmp_fn_t cmp, size_t start,
                 size_t end)
{
...
static void
cc_array_sort_quick_recur (struct cc_array *self, cc_cmp_fn_t cmp,
...
int
cc_array_sort_quick (struct cc_array *self, cc_cmp_fn_t cmp)
{
  if (cmp == NULL)
    return 1;
...
///-----------------------------------------------------------------------------
int
cc_array_sort_bubble (struct cc_array *self, cc_cmp_fn_t cmp)
{
  size_t i, j, k;
